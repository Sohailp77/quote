<div class="max-w-7xl mx-auto py-10 px-4 sm:px-6 lg:px-8 font-sans pb-32">
    ...
    <div wire:loading wire:target="save" class="fixed inset-0 bg-slate-900/10 backdrop-blur-sm z-[100] flex items-center justify-center pointer-events-none transition-opacity duration-300">
    ...
    <div class="mb-8">
        ...
        <p class="text-sm font-bold text-slate-400 dark:text-slate-500 mt-2">Build a professional quote for your customer.</p>
    </div>

    @if(session('success'))
        <div class="mb-6 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 p-4 rounded-2xl flex items-center justify-between border border-emerald-100 dark:border-emerald-800">
    ...
    @if(session('error'))
        <div class="mb-6 bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 p-4 rounded-2xl flex items-center justify-between border border-red-100 dark:border-red-800">
    ...
    <form wire:submit="save" class="space-y-6 relative">
        <!-- 1. Customer Details -->
        <div class="bg-white dark:bg-slate-900 rounded-[32px] shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
            <div class="px-6 py-5 border-b border-slate-100 dark:border-slate-800 flex items-center gap-4 bg-slate-50/50 dark:bg-slate-800/50">
                <div class="w-10 h-10 bg-indigo-50 dark:bg-indigo-900/30 border border-indigo-100 dark:border-indigo-800 text-indigo-600 dark:text-indigo-400 rounded-2xl flex items-center justify-center">
            ...
            <div class="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" wire:model.blur="customer_name" required
                ...
                <div>
                    <label class="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 mb-2 block">Phone Number</label>
                    <div class="relative">
                ...
                <div>
                    <label class="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 mb-2 block">Email Address</label>
                    <div class="relative">
                    ...
                    @error('customer_email') <p class="text-red-500 text-xs mt-1 font-bold">{{ $message }}</p> @enderror
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-[32px] shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
            ...
            <div class="p-0">
                @if(count($items) > 0)
                    <div class="hidden lg:grid grid-cols-12 gap-4 px-6 py-4 bg-slate-50/50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">
                ...
                <div class="divide-y divide-slate-100 dark:divide-slate-800">
                    @foreach($items as $index => $item)
                        @php
                            ...
                            $imagePath = null;
                            if ($variantRef && !empty($variantRef['image_path'])) $imagePath = $variantRef['image_path'];
                            elseif ($productRef && !empty($productRef['image_path'])) $imagePath = $productRef['image_path'];
                            elseif ($productRef && !empty($productRef['category']['image_path'])) $imagePath = $productRef['category']['image_path'];
                        @endphp
                        <div wire:key="item-{{ $item['id'] }}" class="p-6 relative group hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                                <div class="lg:col-span-4 flex gap-4">
                                    ...
                                    <div class="w-12 h-12 rounded-2xl bg-slate-100 dark:bg-slate-800 border-2 border-slate-50 dark:border-slate-700 flex items-center justify-center overflow-hidden shrink-0 mt-1">
                                        @if($imagePath)
                                            <img src="{{ asset($imagePath) }}" class="w-full h-full object-cover">
                                        @else
                                            <x-lucide-package class="w-5 h-5 text-slate-300 dark:text-slate-600" />
                                        @endif
                                    </div>
                                    ...
                                    <div class="flex-1 min-w-0 pr-8 lg:pr-0">
                                        ...
                                        @if($productRef && !empty($productRef['description']))
                                            <p class="mt-2 text-xs text-slate-400 dark:text-slate-500 line-clamp-1 group-hover:line-clamp-none transition-all">
                                ...
                                <div class="lg:col-span-8 flex items-center mt-4 lg:mt-0">
                                    <div class="w-full grid gap-4 items-center {{ $tax_mode === 'item_level' ? 'grid-cols-2 sm:grid-cols-6' : 'grid-cols-2 sm:grid-cols-5' }}">
                                        <!-- Area -->
                                        <div class="col-span-1">
                                            <label class="lg:hidden block text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">Area</label>
                                            ...
                                            @else
                                                <div class="h-11 flex items-center justify-center bg-transparent border-2 border-dashed border-slate-100 dark:border-slate-800 rounded-xl text-slate-300 dark:text-slate-600 text-xs font-black">
                                                    N/A
                                                </div>
                                            @endif
                                        </div>
                                        ...
                                        <!-- Variant -->
                                        <div class="col-span-1">
                                            <label class="lg:hidden block text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">Variant</label>
                                            <select
                                                ...
                                                @if($productRef && !empty($productRef['variants']))
                                                    @foreach($productRef['variants'] as $v)
                                                        <option value="{{ data_get($v, 'id') }}">{{ data_get($v, 'name') }}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                        </div>
                                        ...
                                        <!-- Qty -->
                                        <div class="col-span-1">
                                            <label class="lg:hidden block text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">Qty</label>
                                            <input type="number" min="1"
                                                ...
                                                wire:model.live.debounce.300ms="items.{{ $index }}.quantity" required>
                                        </div>

                                        <!-- Price -->
                                        <div class="col-span-1">
                                            <label class="lg:hidden block text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">Price</label>
                                            <div class="relative">
                                                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 font-bold text-sm">{{ $currency }}</span>
                                                <input type="number" min="0" step="0.01"
                                                    ...
                                                    wire:model.live.debounce.300ms="items.{{ $index }}.price" required>
                                            </div>
                                        </div>
                                        ...
                                        @if($tax_mode === 'item_level')
                                            <div class="col-span-1">
                                                <label class="lg:hidden block text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">Tax</label>
                                                <select
                                                    class="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-xl py-3 px-3 text-sm font-bold text-slate-800 dark:text-slate-200 focus:ring-2 focus:ring-indigo-500/20"
                                                    wire:model.live="items.{{ $index }}.tax_rate_id">
                                                    <option value="">0%</option>
                                                    @foreach($taxRates as $tr)
                                                        <option value="{{ data_get($tr, 'id') }}">{{ data_get($tr, 'name') }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        @endif
                                        ...
                                        <div class="col-span-1 flex items-center justify-end gap-3">
                                            <label class="lg:hidden block text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">Total</label>
                ...
                @if(count($items) === 0)
                    <div class="text-center py-16 border-2 border-dashed border-slate-100 dark:border-slate-800 rounded-3xl m-6">
                        <div class="w-16 h-16 bg-slate-50 dark:bg-slate-800 rounded-2xl flex items-center justify-center mx-auto mb-4 text-slate-300 dark:text-slate-600">
                ...
                @error('items')
                    <div class="m-6 p-4 bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 text-sm font-bold text-center rounded-2xl border border-red-100 dark:border-red-800">
                        {{ $message }}
                    </div>
                @enderror

                @if(count($items) > 0)
                    <div class="flex justify-center p-6 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/50">
                        <button type="button" wire:click="addItem"
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div class="lg:col-span-7">
                <div class="bg-white dark:bg-slate-900 rounded-[32px] shadow-sm border border-slate-200 dark:border-slate-800 p-8 h-full">
                    <div class="flex items-center gap-4 mb-6">
                        <div class="w-10 h-10 bg-sky-50 dark:bg-sky-900/30 border border-sky-100 dark:border-sky-800 text-sky-600 dark:text-sky-400 rounded-2xl flex items-center justify-center">
                    ...
                    <p class="text-[10px] uppercase tracking-widest font-black text-slate-400 dark:text-slate-500 mt-4 flex items-center gap-2">
            ...
            <div class="lg:col-span-5">
                <div class="bg-slate-900 dark:bg-slate-800 text-white rounded-[32px] shadow-xl overflow-hidden shadow-slate-900/10">
                    ...
                    <div class="p-8 space-y-5">
                        ...
                        <div class="flex gap-4 items-center">
                            <div class="w-full flex justify-between items-center text-sm border-t border-white/10 pt-5">
                                <div class="flex items-center gap-4">
                                    <span class="text-slate-400 font-bold uppercase tracking-widest text-[10px]">Discount</span>
                                    <div class="relative w-24">
                                        ...
                                        <span class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 font-black text-xs">%</span>
                                    </div>
                                </div>
                        ...
                        <div class="flex justify-between items-center text-sm border-t border-white/10 pt-5">
                            ...
                            <div class="bg-black/20 p-1 rounded-xl flex text-[10px] font-black uppercase tracking-widest">
                                ...
                                <button type="button" wire:click="setTaxMode('item_level')"
                                    class="{{ $tax_mode === 'item_level' ? 'bg-white text-slate-900 shadow-md' : 'text-slate-400 hover:text-white' }} px-4 py-2 rounded-lg transition-all">Per Item</button>
                            </div>
                        </div>
                        ...
                        <div class="pt-8 mt-6 border-t border-dashed border-white/20">
                            <div class="flex justify-between items-end">
                                <div>
                                    <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Total Amount</span>
                                ...
                                <span class="text-5xl font-black tracking-tighter text-white drop-shadow-lg">
                                    <span class="text-2xl text-slate-400 align-top mr-1 inline-block mt-2">{{ $currency }}</span>
        ...
        <!-- Sticky Action Bar -->
        <div class="fixed bottom-0 left-0 right-0 bg-white/80 dark:bg-slate-900/80 backdrop-blur-2xl border-t border-slate-200 dark:border-slate-800 p-4 shadow-[0_-10px_40px_rgba(0,0,0,0.04)] z-50">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row justify-between items-center gap-4">
                <div class="hidden sm:flex items-center gap-8">
                    <div class="flex items-center gap-3 text-slate-500 dark:text-slate-400">
                        <x-lucide-package class="w-5 h-5 text-indigo-500" />
                        <span class="text-sm font-bold uppercase tracking-widest">
                            <span class="text-slate-900 dark:text-white font-black text-lg">{{ count($items) }}</span> Items
                        </span>
                    </div>
                    <div class="h-8 w-px bg-slate-200 dark:bg-slate-800"></div>
                    <div class="text-[10px] text-slate-400 dark:text-slate-500 font-black uppercase tracking-widest flex items-center gap-3">
                        Total: 
