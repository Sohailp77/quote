<div class="bg-[#f8fafc] dark:bg-slate-950 min-h-screen rounded-[40px] p-6 lg:p-10 font-sans text-slate-800 dark:text-slate-200 relative xl:overflow-hidden transition-colors duration-500">
    ...
    <div class="relative z-10">
        ...
        <div class="flex items-center justify-between mb-8 gap-4">
            ...
            <div class="flex items-center gap-2">
                ...
                <div class="hidden md:flex bg-slate-100 dark:bg-slate-800 p-1 rounded-full">
                    <button wire:click="setTimeframe('weekly')" class="px-3 py-1.5 text-xs font-semibold rounded-full transition-colors {{ $timeframe === 'weekly' ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200' }}">This Week</button>
                    <button wire:click="setTimeframe('monthly')" class="px-3 py-1.5 text-xs font-semibold rounded-full transition-colors {{ $timeframe === 'monthly' ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200' }}">This Month</button>
                    <button wire:click="setTimeframe('yearly')" class="px-3 py-1.5 text-xs font-semibold rounded-full transition-colors {{ $timeframe === 'yearly' ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200' }}">This Year</button>
                    <button wire:click="setTimeframe('all')" class="px-3 py-1.5 text-xs font-semibold rounded-full transition-colors {{ $timeframe === 'all' ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200' }}">All Time</button>
                </div>
        ...
        <!-- Loader Overlay -->
        <div wire:loading.flex class="fixed inset-0 z-50 bg-slate-900/10 backdrop-blur-sm items-center justify-center transition-opacity">
            <div class="bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-xl flex items-center gap-3">
                <x-lucide-loader-2 class="w-6 h-6 text-brand-500 animate-spin" />
                <span class="font-bold text-slate-700 dark:text-slate-200 text-sm">Loading Metrics...</span>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div class="lg:col-span-7 flex flex-col gap-8">
                ...
                <div class="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border border-white/50 dark:border-slate-800 rounded-[32px] p-7 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all duration-300">
                    ...
                    <!-- Disable Livewire updating DOM block here -->
                    <div wire:ignore>
