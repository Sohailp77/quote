        ...
        <div
            ...
            <div class="flex items-center gap-2 flex-shrink-0">
                ...
                <div class="relative hidden lg:block">
                    ...
                    <x-lucide-search
                        class="absolute right-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400 dark:text-slate-500" />
                </div>
                ...
                <div class="relative" @click.away="showUserMenu = false">
                    ...
                    <div x-show="showUserMenu" style="display: none;"
                        ...
                        <div class="px-4 py-3 border-b border-slate-100 dark:border-slate-800">
                            <p class="text-xs font-semibold text-slate-900 dark:text-white">{{ Auth::user()->name }}
                            </p>
                            <p class="text-xs text-slate-400 dark:text-slate-500 truncate">{{ Auth::user()->email }}
                            </p>
                        </div>

                        <div class="px-4 py-2.5 border-b border-slate-100 dark:border-slate-800">
                            <p
                                class="text-[11px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-2">
                                Theme</p>
                            <div class="flex items-center bg-slate-100 dark:bg-slate-800/50 rounded-lg p-1">
                                <button @click="setTheme('light')"
                                    :class="theme === 'light' ?
                                        'bg-white dark:bg-slate-700 shadow-sm text-brand-600 dark:text-brand-400' :
                                        'text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200'"
                                    class="flex-1 flex justify-center py-1.5 rounded-md text-xs font-medium transition-all"
                                    title="Light">
